...
const countries = [
  { code: "BD", dial: "+880", label: "🇧🇩 BD" },
  { code: "IN", dial: "+91", label: "🇮🇳 IN" },
  { code: "PK", dial: "+92", label: "🇵🇰 PK" },
...
export default function AuthPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    setRloading(true);
    try {
      const e164 = phoneObj.number; // +XXXXXXXXX
      const payload = {
        ...
        {mode === "login" ? (
          <form onSubmit={handleLogin} className="px-6 py-6 space-y-4">
            <div>
              ...
              <input id="lemail" type="email" className="w-full rounded-lg border px-4 py-2.5 bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700" value={lemail} onChange={(e) => setLemail(e.target.value)} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1" htmlFor="lpassword">Password</label>
              <input id="lpassword" type="password" className="w-full rounded-lg border px-4 py-2.5 bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700" value={lpassword} onChange={(e) => setLpassword(e.target.value)} />
            </div>
            {lerror && <div className="text-sm text-red-600">{lerror}</div>}
            <button disabled={lloading} className="hover:cursor-pointer w-full rounded-lg bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white font-semibold px-6 py-2.5">{lloading ? "Signing in..." : "Sign in"}</button>
          </form>
        ) : (
